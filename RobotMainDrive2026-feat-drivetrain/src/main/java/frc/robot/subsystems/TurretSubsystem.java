...
public class TurretSubsystem extends SubsystemBase {
    ...
    public void updateAimAngle(Pose2d robotPose, Translation2d targetPos) {
        ...
        m_targetAngleDeg = MathUtil.inputModulus(
            Math.toDegrees(Math.atan2(dy, dx)) - robotPose.getRotation().getDegrees(),
            -180.0, 180.0);
    }
    ...
    public double getDistanceToTarget(Pose2d robotPose, Translation2d targetPos) {
        return Math.hypot(targetPos.getX() - robotPose.getX(), targetPos.getY() - robotPose.getY());
    }
    ...
    public void periodic() {
        if (m_autoAimEnabled) {
            ...
            if ((currentAngle >= TurretConstants.kMaxAngleDeg && output > 0) ||
                (currentAngle <= TurretConstants.kMinAngleDeg && output < 0)) output = 0.0;
