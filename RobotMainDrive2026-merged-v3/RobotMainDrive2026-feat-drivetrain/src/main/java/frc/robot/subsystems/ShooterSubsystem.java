...
public class ShooterSubsystem extends SubsystemBase {
    ...
    public ShooterSubsystem() {
        ...
        m_hoodSolenoid = new DoubleSolenoid(
        ...
        for (double[] row : ShooterConstants.kFlywheelRPMTable) {
            m_flywheelTable.put(row[0], row[1]);
        }
    }
    ...
    public void setDistanceTarget(double distanceMeters) {
        ...
        m_hoodPosition = distanceMeters < ShooterConstants.kHoodSwitchDistanceMeters
            ? HoodPosition.LOW : HoodPosition.HIGH;
    ...
    public void setRPMTrim(double trimPercent) {
        m_rpmTrimPercent = MathUtil.clamp(trimPercent, -10.0, 10.0);
    }
    ...
    public void periodic() {
        if (m_shooterActive) {
            ...
            double output = ShooterConstants.kFlywheelFF * m_targetRPM
                          + ShooterConstants.kFlywheelP  * (m_targetRPM - getFlywheelRPM());
            m_flywheelMotor.set(MathUtil.clamp(output, 0, 1));
        }
